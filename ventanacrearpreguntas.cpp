    ...
    if (!archivo.isEmpty()) {
        ...
        if (!imagen.isNull()) {
            ...
            if (ArchivoBase64.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                ui->label_imagen->setPixmap(
                    imagen.scaled(ui->label_imagen->size(), Qt::KeepAspectRatio));
...
void VentanaCrearPreguntas::on_pushButton_crearPregunta_clicked()
{
    ...
    } else if (text_respuestaCorrecta != "a" && text_respuestaCorrecta != "b"
               && text_respuestaCorrecta != "c") {
        QMessageBox::warning(this,
                             tr("ADVERTENCIA"),
                             tr("Debe ingresar la letra de la opcion correcta (A, B o C)"));
        return;
    }

    preguntas preguntaNueva(text_pregunta,
                            text_opcionA,
                            text_opcionB,
                            text_opcionC,
                            text_respuestaCorrecta,
                            text_categoria,
                            QUuid::createUuid().toString(QUuid::WithoutBraces),
                            imagen_base64);

    std::vector<preguntas> listaDePreguntas;
