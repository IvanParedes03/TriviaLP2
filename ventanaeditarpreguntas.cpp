        ...
        if (!nombreArchivoBase64.isEmpty()) {
            ...
            if (archivoBase64.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                if (imagen.loadFromData(base64Data)) {
                    ui->label_imagenEditar->setPixmap(
                        QPixmap::fromImage(imagen).scaled(ui->label_imagenEditar->size(),
                                                          Qt::KeepAspectRatio));
...
void VentanaEditarPreguntas::on_pushButton_editarPregunta_clicked()
{
    ...
    if (ui->label_imagenEditar->property("imagenBase64File").isValid()) {
        preguntaEditada["imagenBase64File"] = ui->label_imagenEditar->property("imagenBase64File")
                                                  .toString();

    } else {
        QJsonObject preguntaOriginal = preguntaEditando;
        if (preguntaOriginal.contains("imagenBase64File")) {
            preguntaEditada["imagenBase64File"] = preguntaOriginal.value("imagenBase64File")
                                                      .toString();
        }
    }
...
void VentanaEditarPreguntas::on_pushButton_buscarImagenEditar_clicked()
{
    ...
    if (!archivo.isEmpty()) {
        ...
        if (!imagen.isNull()) {
            ...
            QString nuevoNombreArchivo = QUuid::createUuid().toString(QUuid::WithoutBraces)
                                         + ".txt";
            QString rutaCarpetaBase64 = "imagenes_base64/";
            ...
            QString nuevaRutaArchivoBase64 = carpetaBase64.absolutePath() + "/"
                                             + nuevoNombreArchivo;

            if (ui->label_imagenEditar->property("imagenBase64File").isValid()) {
                QString nombreArchivoAnterior = ui->label_imagenEditar->property("imagenBase64File")
                                                    .toString();
            ...
            if (archivoBase64.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                ui->label_imagenEditar->setPixmap(
                    imagen.scaled(ui->label_imagenEditar->size(), Qt::KeepAspectRatio));
                ui->label_imagenEditar->setAlignment(Qt::AlignCenter);
            } else {
                QMessageBox::warning(this,
                                     tr("Error"),
                                     tr("No se pudo guardar la imagen Base64 en el archivo."));
            }
        } else {
